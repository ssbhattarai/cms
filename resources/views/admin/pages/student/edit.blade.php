...
<div class="content-wrapper">
    ...
    @if (count($users) > 0)
      ...
      <div class="row m-3">
        <div class="offset-sm-1 col-sm-5">
            ...
            <div class="form-group">
            <label for="names">Student Name{{$student->name}}</label>
            <select class="js-example-basic-single form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" style="width: 100%">
                  <option value>Select Name</option>
                    @foreach ($users as $user)
                        <option value = "{{ $student->name }}" {{ ($student->name == $user->name) ? "selected" : '' }}>{{ $user->name }}</option>
                  @endforeach
                  </select>
              ...
              <div class="form-group">
                ...
                <select class="js-example-basic-single form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" name="email" style="width: 100%" disabled>
                  <option value>Select Name</option>
                    @foreach ($users as $user)
                    <option value = "{{ $student->email }}" {{ ($student->email == $user->email) ? "selected" : '' }}>{{ $user->email }}</option>
                  @endforeach
                </select>
                <small class="form-text text-muted">You Must Create a Student Login ID First</small>
              ...
              <input type="text" class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" id="phone" placeholder="Enter Phone" name="phone_number" value="{{ $student->phone_number }}">
              ...
              <input type="text" class="form-control {{ $errors->has('permanent_full_address') ? 'is-invalid' : '' }}" id="peraddress" placeholder="Enter Full Address" name="permanent_full_address" value= "{{ $student->permanent_full_address }}">
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control {{ $errors->has('current_full_address') ? 'is-invalid' : '' }}" id="curaddress" placeholder="Enter Full Address" name="current_full_address" value= "{{ $student->current_full_address }}">
      ...
      @endif
              ...
              <div class="form-group">
                ...
                <select class="js-example-basic-single form-control {{ $errors->has('cast') ? 'is-invalid' : '' }}" name="cast" style="width: 100%">
                  <option value>Select cast</option>
                    <option value="Brahmin" @if( $student->cast == "Brahmin") {{ 'selected' }} @endif>Brahmin</option>
                    <option value="Chhetri" @if(  $student->cast == "Chhetri") {{ 'selected' }} @endif>Chhetri</option>
                    <option value="Janajati" @if( $student->cast == "Janajati") {{ 'selected' }} @endif>Janajati</option>
                    <option value="Others" @if( $student->cast == "Others") {{ 'selected' }} @endif>Others</option>
                  </select>
                @if ($errors->has('cast'))
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control {{ $errors->has('passed_college_name') ? 'is-invalid' : '' }}" id="college_name" placeholder="Enter College Name" name="passed_college_name" value= "{{ $student->passed_college_name }}">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control {{ $errors->has('passed_year') ? 'is-invalid' : '' }}" id="passedyear" placeholder="Passed Year" name="passed_year" value= "{{ $student->passed_year }}">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control {{ $errors->has('marks_obtain') ? 'is-invalid' : '' }}" id="obtainedMarks" placeholder="Enter Marks" name="marks_obtain" value= "{{ $student->marks_obtain }}">
        ...
        <button type="submit" class="btn btn-outline-primary ml-10 btn-lg mb-4" style="margin-left: 41%; border-radius: 20px;">Update Student</button>
      </form>
    @endif
      {{-- @foreach ($users as $user) --}}
          @if ( count($users) <= 0 )
